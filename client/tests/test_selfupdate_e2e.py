...
Одни и те же сценарии гоняются под Windows (exe) и под Linux (бинарник без
расширения, собранный в python:3.11-buster).

Запускается только если заданы переменные окружения (в CI — джобы
selfupdate-e2e-windows и selfupdate-e2e-linux в .github/workflows/ci.yml),
иначе пропускается:
...
Что проверяется на живом бинарнике (в отличие от юнит-тестов):
  ...
  - релиз запрашивается для своей ОС: стаб отдаёт его только агенту,
    приславшему верный X-Client-Platform;
