...
Стаб сервера релизов ОБЯЗАН слушать 127.0.0.1:8000: упакованный exe не читает
config.json, а переменной окружения для server_ingest_url нет, поэтому адрес
сервера всегда встроенный http://127.0.0.1:8000/api/ingest (см. README).
